...
from bark.projects.schemas import (
    InputQuote as InputQuoteSchema,
    OutputQuote as OutputQuoteSchema,
    SortOrder,
    QuoteStatus,
)
...
def create_quote(project_id: int, quote: InputQuoteSchema, db=Depends(get_session)):
    ...
    if project.response_count >= project.response_cap:
        ...
        raise HTTPException(
            status_code=400, detail="Bark already reach max response cap"
        )

    existing_project_quote = (
        db.query(QuoteModel)
        .filter(
            QuoteModel.seller_id == quote.seller_id, QuoteModel.project_id == project_id
        )
        .first()
    )
    if existing_project_quote:
        raise HTTPException(
            status_code=400, detail="Seller already has a quote for this bark"
        )

    quote = QuoteModel(project_id=project_id, **quote.model_dump())
    db.add(quote)
...
def list_bark_quotes(
    ...
    if price_cents_sort == "asc":
        project_quotes_query = project_quotes_query.order_by(
            QuoteModel.price_cents.asc()
        )
    else:
        project_quotes_query = project_quotes_query.order_by(
            QuoteModel.price_cents.desc()
        )
...
def accept_quote(project_id: int, quote_id: int, db=Depends(get_session)):
    ...
    if existing_accepted_quotes:
        raise HTTPException(
            status_code=400, detail="Project already has accepted quotes"
        )

    quote_to_be_accepted = db.execute(
        select(QuoteModel)
        .where(QuoteModel.id == quote_id)
        .where(QuoteModel.project_id == project_id)
        .with_for_update()
    ).scalar_one_or_none()
